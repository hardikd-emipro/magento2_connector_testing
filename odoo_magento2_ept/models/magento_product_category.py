...
class MagentoProductCategoryEpt(models.Model):
    """
        Describes Magento Product Category
    """
    _name = "magento.product.category.ept"
    ...
    def _compute_complete_name(self):
        for category in self:
            ...
            if parent_id:
                complete_name = "{} / {}".format(parent_id.complete_category_name, category.name)
    ...
    magento_parent_id = fields.Many2one(
        'magento.product.category.ept',
        'Magento Category',
        help="Magento Parent Category ID",
        ondelete='cascade'
    )
    magento_child_ids = fields.One2many(comodel_name='magento.product.category.ept',
    ...
    def _get_parent_category(self, instance, category):
        ...
        if parent_id:
            # Search the parent category in magento.product.category.ept
    ...
    def create_magento_category(self, instance, category):
        ...
        if category.get('children_data', list()):
    ...
    def get_magento_product_category(instance, parent_id=0):
        ...
        :param parent_id: Parent magento.product.category.ept id
        :return: dict(category response)
        """
        url = '/V1/categories'
        if parent_id:
            url = "{}/{}".format(url, parent_id)
        try:

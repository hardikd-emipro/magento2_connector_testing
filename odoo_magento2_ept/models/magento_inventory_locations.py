...
class MagentoInventoryLocations(models.Model):
    ...
    code = fields.Char(string="Magento Location Code", readonly=True, help="Store view Code")
    ...
    export_stock_warehouse_ids = fields.Many2many(
        'stock.warehouse',
        string="Export Stock Warehouses",
        ...
        'It will compute the stock quantities from those Warehouses and export them to the current source location in the Magento.'
    )
    ...
    ship_from_location = fields.Many2one(
        'stock.location', string='Export Shipment Location',
    ...
    def _check_locations_warehouse_ids(self):
        ...
        for location in locations:
            if any([location in location_warehouse.ids for location in location.export_stock_warehouse_ids.ids]):
                raise ValidationError(_("Cann't set this warehouse in "
    ...
    def _check_ship_from_location(self):
        ...
        for inv_loc in inv_locs:
            if inv_locs and inv_loc.id != self.id:
                raise ValidationError(_("It is not possible to set this location multiple times in the same "
